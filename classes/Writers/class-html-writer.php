	...
	class HTML_Writer {
		...
		private static function build_report_header( $report_data ): string {
			...
			if ( \class_exists( 'WSAL\Extensions\Views\Reports' ) ) {
				...
				$logo_link      = $white_settings['logo_url'];
				...
				$contact_attributes = array(
					esc_html__( 'Business Name', 'wp-security-audit-log' ) => $white_settings['business_name'],
					esc_html__( 'Contact Name', 'wp-security-audit-log' ) => $white_settings['name_surname'],
					esc_html__( 'Contact Email', 'wp-security-audit-log' ) => $white_settings['email'],
					esc_html__( 'Contact Phone', 'wp-security-audit-log' ) => $white_settings['phone_number'],
				);
