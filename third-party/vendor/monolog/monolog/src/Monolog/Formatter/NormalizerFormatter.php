 ...
 */
class NormalizerFormatter implements \WSAL_Vendor\Monolog\Formatter\FormatterInterface
{
    ...
    /** @var int */
    private $jsonEncodeOptions = \WSAL_Vendor\Monolog\Utils::DEFAULT_JSON_FLAGS;
    /**
    ...
    protected function normalize($data, int $depth = 0)
    {
        ...
        if (\is_object($data)) {
            ...
            return [\WSAL_Vendor\Monolog\Utils::getClass($data) => $value];
        }
        if (\is_resource($data)) {
    ...
    protected function normalizeException(\Throwable $e, int $depth = 0)
    {
        ...
        $data = ['class' => \WSAL_Vendor\Monolog\Utils::getClass($e), 'message' => $e->getMessage(), 'code' => (int) $e->getCode(), 'file' => $e->getFile() . ':' . $e->getLine()];
    ...
    {
        return \WSAL_Vendor\Monolog\Utils::jsonEncode($data, $this->jsonEncodeOptions, $ignoreErrors);
    }
    /**
     * @return string
     */
    protected function formatDate(\DateTimeInterface $date)
    {
        ...
        if ($this->dateFormat === self::SIMPLE_DATE && $date instanceof \WSAL_Vendor\Monolog\DateTimeImmutable) {
            return (string) $date;
        }
