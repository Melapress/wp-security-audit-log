 ...
 */
class SyslogUdpHandler extends \WSAL_Vendor\Monolog\Handler\AbstractSyslogHandler
{
    const RFC3164 = 0;
    const RFC5424 = 1;
    const RFC5424e = 2;
     ...
     */
    public function __construct(string $host, int $port = 514, $facility = \LOG_USER, $level = \WSAL_Vendor\Monolog\Logger::DEBUG, bool $bubble = \true, string $ident = 'php', int $rfc = self::RFC5424)
    {
        parent::__construct($facility, $level, $bubble);
        $this->ident = $ident;
        $this->rfc = $rfc;
        $this->socket = new \WSAL_Vendor\Monolog\Handler\SyslogUdp\UdpSocket($host, $port);
    }
    ...
    private function splitMessageIntoLines($message) : array
    {
        ...
        if (\false === $lines) {
            ...
            throw new \RuntimeException('Could not preg_split: ' . $pcreErrorCode . ' / ' . \WSAL_Vendor\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $lines;
    }
    /**
     ...
     */
    public function setSocket(\WSAL_Vendor\Monolog\Handler\SyslogUdp\UdpSocket $socket) : self
    {
        $this->socket = $socket;
        return $this;
    }
