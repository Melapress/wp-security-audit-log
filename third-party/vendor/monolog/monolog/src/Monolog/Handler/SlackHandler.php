 ...
 */
class SlackHandler extends \WSAL_Vendor\Monolog\Handler\SocketHandler
{
    /**
     * Slack API token
     * @var string
     ...
     */
    public function __construct(string $token, string $channel, ?string $username = null, bool $useAttachment = \true, ?string $iconEmoji = null, $level = \WSAL_Vendor\Monolog\Logger::CRITICAL, bool $bubble = \true, bool $useShortAttachment = \false, bool $includeContextAndExtra = \false, array $excludeFields = array())
    {
        if (!\extension_loaded('openssl')) {
            throw new \WSAL_Vendor\Monolog\Handler\MissingExtensionException('The OpenSSL PHP extension is required to use the SlackHandler');
        }
    ...
    protected function prepareContentData(array $record) : array
    {
        ...
        if (!empty($dataArray['attachments'])) {
            $dataArray['attachments'] = \WSAL_Vendor\Monolog\Utils::jsonEncode($dataArray['attachments']);
        }
        return $dataArray;
    }
    /**
