 ...
 */
class PushoverHandler extends \WSAL_Vendor\Monolog\Handler\SocketHandler
{
    /** @var string */
    private $token;
     ...
     */
    public function __construct(string $token, $users, ?string $title = null, $level = \WSAL_Vendor\Monolog\Logger::CRITICAL, bool $bubble = \true, bool $useSSL = \true, $highPriorityLevel = \WSAL_Vendor\Monolog\Logger::CRITICAL, $emergencyLevel = \WSAL_Vendor\Monolog\Logger::EMERGENCY, int $retry = 30, int $expire = 25200)
    {
        ...
        $this->highPriorityLevel = \WSAL_Vendor\Monolog\Logger::toMonologLevel($highPriorityLevel);
        $this->emergencyLevel = \WSAL_Vendor\Monolog\Logger::toMonologLevel($emergencyLevel);
        $this->retry = $retry;
        $this->expire = $expire;
    }
    ...
    private function buildContent(array $record) : string
    {
        ...
        $message = \WSAL_Vendor\Monolog\Utils::substr($message, 0, $maxMessageLength);
    ...
    {
        $this->highPriorityLevel = \WSAL_Vendor\Monolog\Logger::toMonologLevel($value);
        return $this;
    }
    /**
     * @param int|string $value
    ...
    {
        $this->emergencyLevel = \WSAL_Vendor\Monolog\Logger::toMonologLevel($value);
        return $this;
    }
    /**
