 ...
 */
class FleepHookHandler extends \WSAL_Vendor\Monolog\Handler\SocketHandler
{
    ...
    public function __construct(string $token, $level = \WSAL_Vendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\extension_loaded('openssl')) {
            throw new \WSAL_Vendor\Monolog\Handler\MissingExtensionException('The OpenSSL PHP extension is required to use the FleepHookHandler');
        }
        $this->token = $token;
    ...
    {
        return new \WSAL_Vendor\Monolog\Formatter\LineFormatter(null, null, \true, \true);
    }
    /**
