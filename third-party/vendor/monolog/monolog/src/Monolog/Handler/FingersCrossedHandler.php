 ...
 */
class FingersCrossedHandler extends \WSAL_Vendor\Monolog\Handler\Handler implements \WSAL_Vendor\Monolog\Handler\ProcessableHandlerInterface, \WSAL_Vendor\Monolog\ResettableInterface, \WSAL_Vendor\Monolog\Handler\FormattableHandlerInterface
{
    ...
    public function __construct($handler, $activationStrategy = null, int $bufferSize = 0, bool $bubble = \true, bool $stopBuffering = \true, $passthruLevel = null)
    {
        if (null === $activationStrategy) {
            $activationStrategy = new \WSAL_Vendor\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy(\WSAL_Vendor\Monolog\Logger::WARNING);
        }
        // convert simple int activationStrategy to an object
        if (!$activationStrategy instanceof \WSAL_Vendor\Monolog\Handler\FingersCrossed\ActivationStrategyInterface) {
            $activationStrategy = new \WSAL_Vendor\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy($activationStrategy);
        }
        ...
        if ($passthruLevel !== null) {
            $this->passthruLevel = \WSAL_Vendor\Monolog\Logger::toMonologLevel($passthruLevel);
        }
