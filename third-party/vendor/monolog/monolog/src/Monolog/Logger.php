 ...
 */
class Logger implements \WSAL_Vendor\Psr\Log\LoggerInterface, \WSAL_Vendor\Monolog\ResettableInterface
{
    /**
     ...
     */
    public function pushHandler(\WSAL_Vendor\Monolog\Handler\HandlerInterface $handler) : self
    {
     ...
     */
    public function popHandler() : \WSAL_Vendor\Monolog\Handler\HandlerInterface
    {
        if (!$this->handlers) {
    ...
    public function addRecord(int $level, string $message, array $context = []) : bool
    {
        ...
        foreach ($this->handlers as $handler) {
            if (null === $record) {
                ...
                $record = ['message' => $message, 'context' => $context, 'level' => $level, 'level_name' => $levelName, 'channel' => $this->name, 'datetime' => new \WSAL_Vendor\Monolog\DateTimeImmutable($this->microsecondTimestamps, $this->timezone), 'extra' => []];
                try {
    ...
    public function reset() : void
    {
        ...
        foreach ($this->processors as $processor) {
            if ($processor instanceof \WSAL_Vendor\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
    ...
    public static function getLevelName(int $level) : string
    {
        if (!isset(static::$levels[$level])) {
            throw new \WSAL_Vendor\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
    ...
    public static function toMonologLevel($level) : int
    {
        if (\is_string($level)) {
            ...
            throw new \WSAL_Vendor\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels) + static::$levels));
        }
        if (!\is_int($level)) {
            throw new \WSAL_Vendor\Psr\Log\InvalidArgumentException('Level "' . \var_export($level, \true) . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels) + static::$levels));
        }
        return $level;
    }
    /**
