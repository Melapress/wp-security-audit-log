...
class CurlClient implements \WSAL_Vendor\Twilio\Http\Client
{
    ...
    public function request(string $method, string $url, array $params = [], array $data = [], array $headers = [], string $user = null, string $password = null, int $timeout = null) : \WSAL_Vendor\Twilio\Http\Response
    {
        ...
        try {
            if (!($curl = \curl_init())) {
                throw new \WSAL_Vendor\Twilio\Exceptions\EnvironmentException('Unable to initialize cURL');
            }
            if (!\curl_setopt_array($curl, $options)) {
                throw new \WSAL_Vendor\Twilio\Exceptions\EnvironmentException(\curl_error($curl));
            }
            ...
            $this->lastResponse = new \WSAL_Vendor\Twilio\Http\Response($statusCode, $body, $responseHeaders);
            ...
            case 'put':
                ...
                if ($data) {
                    if ($buffer = \fopen('php://memory', 'w+')) {
                        ...
                        throw new \WSAL_Vendor\Twilio\Exceptions\EnvironmentException('Unable to open a temporary file');
                    }
                }
                break;
            case 'head':
