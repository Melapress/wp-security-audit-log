...
class CloudWatch extends \WSAL_Vendor\Monolog\Handler\AbstractProcessingHandler
{
    /**
     ...
     * @throws \Exception
     */
    public function __construct(\Aws\CloudWatchLogs\CloudWatchLogsClient $client, $group, $stream, $retention = 14, $batchSize = 10000, array $tags = [], $level = \WSAL_Vendor\Monolog\Logger::DEBUG, $bubble = \true, $createGroup = \true)
    {
        if ($batchSize > 10000) {
    ...
    {
        return new \WSAL_Vendor\Monolog\Formatter\LineFormatter("%channel%: %level_name%: %message% %context% %extra%", null, \false, \true);
    }
    /**
     * {@inheritdoc}
     */
