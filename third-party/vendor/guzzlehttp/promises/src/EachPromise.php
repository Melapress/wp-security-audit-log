 ...
 */
class EachPromise implements \WSAL_Vendor\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    ...
    {
        $this->iterable = \WSAL_Vendor\GuzzleHttp\Promise\Create::iterFor($iterable);
    ...
    {
        $this->mutex = \false;
        $this->aggregate = new \WSAL_Vendor\GuzzleHttp\Promise\Promise(function () {
            \reset($this->pending);
            ...
            while ($promise = \current($this->pending)) {
                \next($this->pending);
                $promise->wait();
                if (\WSAL_Vendor\GuzzleHttp\Promise\Is::settled($this->aggregate)) {
                    return;
                }
            }
        });
    ...
    private function addPending()
    {
        ...
        $promise = \WSAL_Vendor\GuzzleHttp\Promise\Create::promiseFor($this->iterable->current());
